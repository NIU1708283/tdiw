...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Fragment a completar dins de iniciarsesio.php (després de pg_query_params)
            if ($res && pg_num_rows($res) > 0) {
            $usuari = pg_fetch_assoc($res);
            // Verificació segura del hash de la contrasenya
                if (password_verify($password, $usuari['password_hash'])) {
                // Guardem les dades mínimes a la sessió
                    $_SESSION['usuari'] = [
                    'id' => $usuari['id'],
                    'nom' => $usuari['nom'],
                    'email' => $email
                    ];
                // Redirecció a la home o al perfil després de login correcte
                header('Location: index.php?action=home&login=success');
                exit();
            } else {
                $errors[] = 'Contrasenya incorrecta.';
            }
        } else {
            $errors[] = 'Aquest correu electrònic no està registrat.';
        }
    if ($res && pg_num_rows($res) > 0) {
